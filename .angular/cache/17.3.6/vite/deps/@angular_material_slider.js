import {
  ...
  Platform,
  RippleState,
} from "./chunk-RVAAU357.js";
import "./chunk-G4OHPOIF.js";
import { Directionality } from "./chunk-TVWQTQX2.js";
import { NG_VALUE_ACCESSOR } from "./chunk-GNI66HDW.js";
...
import { Subject } from "./chunk-WSA2QMXP.js";
...
function MatSlider_Conditional_6_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
    ɵɵrepeaterCreate(
      0,
      MatSlider_Conditional_6_Conditional_2_For_1_Template,
      1,
      4,
      "div",
      8,
...
function MatSlider_Conditional_7_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r2 = ɵɵnextContext();
    ɵɵproperty("discrete", ctx_r2.discrete)("thumbPosition", 1)(
      "valueIndicatorText",
      ctx_r2.startValueIndicatorText,
    );
  }
}
var _MatThumb;
  ...
  _showRipple(animation, ignoreGlobalRippleConfig) {
    ...
    if (this._slider._isRange) {
      const sibling = this._slider._getThumb(
        this.thumbPosition === _MatThumb.START
          ? _MatThumb.END
          : _MatThumb.START,
      );
    ...
    if (
      this._slider._globalRippleOptions?.disabled &&
...
_MatSliderVisualThumb.ɵcmp = ɵɵdefineComponent({
  ...
  viewQuery: function MatSliderVisualThumb_Query(rf, ctx) {
    ...
    if (rf & 2) {
      ...
      ɵɵqueryRefresh((_t = ɵɵloadQuery())) &&
        (ctx._valueIndicatorContainer = _t.first);
    }
  },
  ...
  template: function MatSliderVisualThumb_Template(rf, ctx) {
    if (rf & 1) {
      ɵɵtemplate(
        0,
        MatSliderVisualThumb_Conditional_0_Template,
        5,
        1,
        "div",
        2,
...
var _MatSlider = class _MatSlider {
  ...
  _isActive() {
    return (
      this._getThumb(_MatThumb.START)._isActive ||
      this._getThumb(_MatThumb.END)._isActive
    );
  }
  ...
  _skipUpdate() {
    return !!(
      this._getInput(_MatThumb.START)?._skipUIUpdate ||
      this._getInput(_MatThumb.END)?._skipUIUpdate
    );
  }
  ...
  _calcTickMarkTransform(index) {
    const translateX =
      index * (this._tickMarkTrackWidth / (this._tickMarks.length - 1));
  ...
  _updateOverlappingThumbClassNames(source) {
    ...
    sourceThumb._hostElement.classList.toggle(
  ...
  _updateValueIndicatorUI(source) {
    ...
    if (this.discrete) {
      ...
      valuetext.length < 3
        ? visualThumb._hostElement.classList.add(
            "mdc-slider__thumb--short-value",
          )
        : visualThumb._hostElement.classList.remove(
            "mdc-slider__thumb--short-value",
          );
    }
  }
  ...
  _updateTickMarkUI() {
    ...
    this._isRange
      ? this._updateTickMarkUIRange(step)
      : this._updateTickMarkUINonRange(step);
    if (this._isRtl) {
  ...
  _updateTickMarkUIRange(step) {
    ...
    const numInactiveBeforeStartThumb = Math.max(
      Math.floor((startValue - this.min) / step),
      0,
    );
    const numActive = Math.max(
      Math.floor((endValue - startValue) / step) + 1,
      0,
    );
    const numInactiveAfterEndThumb = Math.max(
      Math.floor((this.max - endValue) / step),
      0,
    );
  ...
  _getInput(thumbPosition) {
    ...
    if (this._inputs?.length) {
      return thumbPosition === _MatThumb.START
        ? this._inputs.first
        : this._inputs.last;
    }
    return;
  ...
  _getThumb(thumbPosition) {
    return thumbPosition === _MatThumb.END
      ? this._thumbs?.last
      : this._thumbs?.first;
  }
  _setTransition(withAnimation) {
    this._hasAnimation =
      !this._platform.IOS && withAnimation && !this._noopAnimations;
...
_MatSlider.ɵcmp = ɵɵdefineComponent({
  ...
  hostBindings: function MatSlider_HostBindings(rf, ctx) {
    if (rf & 2) {
      ...
      ɵɵclassProp("mdc-slider--range", ctx._isRange)(
        "mdc-slider--disabled",
        ctx.disabled,
      )("mdc-slider--discrete", ctx.discrete)(
  ...
  template: function MatSlider_Template(rf, ctx) {
    if (rf & 1) {
      ...
      ɵɵtemplate(
        7,
        MatSlider_Conditional_7_Template,
        1,
        3,
    ...
    if (rf & 2) {
      ...
      ɵɵadvance();
      ɵɵproperty("discrete", ctx.discrete)("thumbPosition", 2)(
        "valueIndicatorText",
        ctx.endValueIndicatorText,
      );
    }
  },
...
function _validateInputs(isRange, endInputElement, startInputElement) {
  const startValid =
    !isRange ||
    startInputElement?._hostElement.hasAttribute("matSliderStartThumb");
...
var _MatSliderRangeThumb = class _MatSliderRangeThumb extends MatSliderThumb {
  ...
  getSibling() {
    if (!this._sibling) {
      this._sibling = this._slider._getInput(
        this._isEndThumb ? _MatThumb.START : _MatThumb.END,
      );
    }
  ...
  _updateWidthActive() {
    const minWidth =
      this._slider._rippleRadius * 2 - this._slider._inputPadding * 2;
    const maxWidth =
...
_MatSliderRangeThumb.ɵfac = function MatSliderRangeThumb_Factory(t) {
  return new (t || _MatSliderRangeThumb)(
    ɵɵdirectiveInject(NgZone),
    ɵɵdirectiveInject(MAT_SLIDER),
    ɵɵdirectiveInject(ElementRef),
    ɵɵdirectiveInject(ChangeDetectorRef),
